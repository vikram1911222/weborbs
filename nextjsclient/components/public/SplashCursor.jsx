...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    // Event handling helpers - UPDATED TO WINDOW + BOUNDS CHECK
    ...
    function handleMouseMove(e) {
      ...
      if (!pointer.moved && pointer.color[0] === 0) {
        pointer.color = generateColor();
      }
    ...
    function handleTouchStart(e) {
      ...
      // Check if any touch is inside
      let inside = false;
      for (let i = 0; i < touches.length; i++) {
        if (isInside(touches[i].clientX, touches[i].clientY)) {
          inside = true;
          break;
        }
      }
      ...
      for (let i = 0; i < touches.length; i++) {
        if (isInside(touches[i].clientX, touches[i].clientY)) {
          const pos = getEventPos(touches[i].clientX, touches[i].clientY);
  ...
  return (
    <div className="absolute inset-0 z-0 pointer-events-none w-full h-full text-white">
      <canvas
        ref={canvasRef}
        id="fluid"
